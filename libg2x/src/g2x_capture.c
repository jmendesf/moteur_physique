...
{ 
  ...
  fflush(framefile);
  fclose(framefile);
	snprintf(command,2047,"rm -f %s",tempsnapname);
  _r_=system(command);
  snprintf(command,2047,"rm -rf %s/",tempdirname);
  _r_=system(command);
...
extern bool g2x_Snapshot(char *format, char *basename, int w, int h)
{
  ...
  if (match_string(format,pnm_name))
    snprintf(command,2047,"pnmflip -tb %s > %s",tempsnapname,snapname);
  else if (match_string(format,bmp_name))
    snprintf(command,2047,"cat %s | pnmflip -tb | ppmtobmp > %s",tempsnapname,snapname);
  else if (match_string(format,jpg_name))
    snprintf(command,2047,"cat %s | pnmflip -tb | pnmtojpeg -quality=100 -dct=float> %s",tempsnapname,snapname);
  else if (match_string(format,gif_name))
    snprintf(command,2047,"cat %s | pnmflip -tb | pnmquant -fs 256 | ppmtogif > %s",tempsnapname,snapname);
  else if (match_string(format,tif_name))
    snprintf(command,2047,"cat %s | pnmflip -tb | pnmtotiff -packbits > %s",tempsnapname,snapname);
  else if (match_string(format,ras_name))
    snprintf(command,2047,"cat %s | pnmflip -tb | pnmtorast > %s",tempsnapname,snapname);
  else if (match_string(format,png_name))
    snprintf(command,2047,"cat %s | pnmflip -tb | pnmtopng -compression 9 -transparent black > %s",tempsnapname,snapname);
  else if (match_string(format,eps_name))
    snprintf(command,2047,"cat %s | pnmflip -tb | pnmtops -rle -dpi 600 > %s",tempsnapname,snapname);
  else 
  {
...
extern bool g2x_FilmFrame(void)
{
  ...
  { 
	...
	fprintf(stderr,"\t\t\b\b%04d\e[0;36m\e[0m\t\r",framenum);
  ...
  snprintf(command,2047,"cat %s | pnmflip -tb | pnmtojpeg -quality=100 -dct=float> %s/frame.%04d.jpg"
...
{
	snprintf(command,2047,"mencoder -quiet mf://%s/*.jpg -mf fps=%d:type=jpg -o %s.mp4 \
-ovc lavc -lavcopts vcodec=mpeg4:vbitrate=%d:mbd=2:o=mpv_flags=+mv0:trell:v4mv:o=mpv_flags=+cbp_rd:last_pred=3:predia=2:dia=2:vmax_b_frames=2:vb_strategy=1:precmp=2:cmp=2:subcmp=2:preme=2:o=quantizer_noise_shaping=2:threads=6"
		       	           ,tempdirname,fps,capturename,bitrate);
  _r_=system(command);
	return true;
}
